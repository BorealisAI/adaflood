...
class VGG(nn.Module):
    def __init__(
        self, name, features: nn.Module, d_model: int = 64, num_classes: int = 1000, init_weights: bool = True, dropout: float = 0.5
    ) -> None:
        super().__init__()
...
def _vgg(cfg: str, batch_norm: bool, weights: Optional[WeightsEnum], progress: bool, **kwargs: Any) -> VGG:
    ...
    model = VGG(features=make_layers(cfgs[cfg], batch_norm=batch_norm), **kwargs)
    if weights is not None:
