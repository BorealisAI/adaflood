...
def main(args):
    ...
    gammas = [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.90, 0.95, 1.0]
    ...
    for i, alpha in enumerate(alphas):
        ...
        if i == 0:
            for method in summary_df['method']:
                new_row = {np.around(alpha_i, 1): None for alpha_i in alphas}
                new_row.update({'method': method})
                result_df = pd.concat([result_df, pd.DataFrame([new_row])], ignore_index=True)

        for j, method in enumerate(summary_df['method']):
            result_df.loc[j, args.alpha] = summary_df[summary_df.method == method]['test_acc'].item()
